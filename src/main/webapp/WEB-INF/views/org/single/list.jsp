...
<body>
	...
	<div class="row">
		<ol class="breadcrumb" style="border-bottom: 1px solid #0088cc;">
			...
			<li><a href="javascript: void(0)">单店列表</a></li>
		</ol>
	</div>
		...
		<div class="row">
			<div class="col-sm-3 g-col-gap s-row">
				<div class="col-sm-1 g-label">
					<label for="inputType" class="control-label s-control-label">单店：</label>
				</div>
				<div class="col-sm-11 g-input">
					<select class="form-control" name="inputType" id="inputType">
						<option value="0">单店全称</option>
						<option value="1">单店账号</option>
					</select>
				</div>
	...
	<div class="row">
		<table data-bt-table data-url="org/singleOrg/list"
			...
			<thead>
				<tr>
					...
					<th data-field="orgAccount" data-formatter="actionFormatter">单店账号</th>
					<th data-field="orgFullName">单店全称</th>
					<th data-field="orgName">单店简称</th>
	...
	function operatorFormatter(value, row, index) {
		var html = "";
		<shiro:hasPermission name="org/singleOrg/edit">
			html += '<button class="btn btn-primary btn-xs" data-url='+basePath+'"org/singleOrg/edit?id=' + row.id + '" data-model="navTab" title="编辑"><i class="glyphicon glyphicon-pencil"></i></button> ';
		</shiro:hasPermission>
		<shiro:hasPermission name="org/singleOrg/resetPwd">
			html += '<button class="btn btn-xs" data-url="org/singleOrg/resetPwd?id=' + row.id + '" data-model="ajaxToDo" data-msg="确认重置该单店的账号密码？，确认后该单店的账号密码被重置为123456" data-targetid="my_delete_modal" title="重置密码"><i class="glyphicon glyphicon-wrench"></i></button> ';
		</shiro:hasPermission>
		<shiro:hasPermission name="org/singleOrg/delete">
			html += '<button class="btn-danger btn-xs" data-url="org/singleOrg/delete?id=' + row.id + '" data-msg="确认删除该单店吗？" data-model="ajaxToDo" data-targetid="my_modal" data-backdrop="static" title="授权"><i class="glyphicon glyphicon-trash"></i></button> ';
		</shiro:hasPermission>
		return html;
	}
	...
	function industryFormatter(value, row, index){
	 ...
	 function actionFormatter(value, row, index) {
         ...
         <shiro:hasPermission name="org/singleOrg/detail">
         	html += '<a style="text-decoration: underline!important;" href="org/singleOrg/detail?id=' + row.id + '"   data-target="navTab" data-targetid="my_modal" data-backdrop="static" title="'+ orgAccount + '">';
         </shiro:hasPermission>
         html += orgAccount;
         <shiro:hasPermission name="org/singleOrg/detail">
         	html += '</a>';
