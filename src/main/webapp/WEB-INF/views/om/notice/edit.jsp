...
<form class="form-horizontal" id="editNoticeForm" action="<%=basePath%>om/notice/edit" data-ajax-after="om/notice"
      method="post">
    ...
    <div class="container-fluid">
        <div class="row padding-top-10">
            ...
            <div class="col-sm-10">
                ...
                <div class="col-sm-10">
                    <input class="form-control isValidata" name="title" id="title" placeholder="1-50位英文、数字、汉字、符号及组合"
                           maxlength="50" value="${notice.title}" autocomplete="off" checknull="请输入公告标题" checkEsAndZhAndNum_1_50="公告标题格式错误" onblur="validateHorizentalForm(1,this)" onfocus="validateHorizentalForm(2,this)">
        ...
        <div class="row padding-top-10">
            <div class="col-sm-1"></div>
            <div class="col-sm-10">
                <label for="sendObject" class="col-sm-1 control-label">发送对象<span
        ...
        <div class="row padding-top-10">
            <div class="col-sm-1"></div>
            <div class="col-sm-10">
                <label for="deviceType" class="col-sm-1 control-label">指定设备<span
                        class="error">*</span>：</label>
                <div class="col-sm-3">
                    <select class="form-control isValidata" id="deviceType" name="deviceType" checknull="请选择指定设备" onblur="validateHorizentalForm(1,this)" onfocus="validateHorizentalForm(2,this)">
                        ...
                        <option value="1"
                                <c:if test="${notice.deviceType==1}">selected="selected"</c:if>>
                            安卓
                        </option>
                        <option value="2"
                                <c:if test="${notice.deviceType==2}">selected="selected"</c:if>>
                            IOS
                        </option>
                    </select>
                    <span style="color:red;padding-left: 5px;float:left;visibility: hidden;">&nbsp;</span>
                        ...
                        <select class="form-control s-form-control" id="province" name="province">
                            ...
                            <c:forEach items="${provinceList}" var="item">
                                <option value="${item.id}"
                                        <c:if test="${item.id == notice.province}">selected="selected"</c:if>>${item.name}</option>
                            </c:forEach>
                        </select>
                    </div>
                    <div class="col-sm-3" style="padding-right:0;">
                        <select class="form-control s-form-control" id="city" name="city">
                            ...
                            <c:forEach items="${cityList}" var="item">
                                <option value="${item.id}"
                                        <c:if test="${item.id == notice.city}">selected="selected"</c:if>>${item.name}</option>
                            </c:forEach>
                        </select>
                    </div>
                    <div class="col-sm-3" style="padding-right:0;">
                        <select class="form-control s-form-control" id="county" name="county">
                            ...
                            <c:forEach items="${countyList}" var="item">
                                <option value="${item.id}"
                                        <c:if test="${item.id == notice.county}">selected="selected"</c:if>>${item.name}</option>
                            </c:forEach>
                        </select>
                    </div>
                </div>
        ...
        <div class="row padding-top-10">
            <div class="col-sm-1"></div>
            <div class="col-sm-10">
                <label for="orgId" class="col-sm-1 control-label">选择地区：</label>
                ...
                <div class="col-sm-7" style="margin-left: -15px;">
                    <div class="col-sm-5">
                        <div class="input-group date form_datetime">
                            <input type="text" class="form-control" name="sendTimeStr"
                                   id="sendTimeStr" readonly="readonly"
                                   style="background-color:white;"
                                   value="<date:date value="${notice.sendTime}"  pattern="yyyy-MM-dd HH:mm"/>" placeholder="请选择">
...
<script type="text/javascript">
    ...
    function loadProvince() {
        ...
        $.ajax({
            url: "/bussiness/address/getProvinces",
            dataType: "json",
    ...
    function loadCity() {
        $("#county").html('<option value="0">请选择区</option>');
        $.ajax({
            url: "/bussiness/address/getCitys?provinceId=" + $("#province").val(),
            dataType: "json",
            success: function (data) {
    ...
    function loadCounty() {
        $.ajax({
            url: "/bussiness/address/getAreas?cityId=" + $("#city").val(),
            dataType: "json",
            success: function (data) {
